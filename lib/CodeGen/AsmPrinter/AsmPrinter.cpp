...
static void emitGlobalConstantDataSequential(const DataLayout &DL,
  ...
  if (isa<IntegerType>(CDS->getElementType())) {
    for (unsigned i = 0, e = CDS->getNumElements(); i != e; ++i) {
      ...
      // SICREAL - HACK - SICHACK - 24 bit ints in structs / arrays
      if (ElementByteSize == 4 && strcmp(AP.TM.getTarget().getName(), "sic") == 0) {
        AP.OutStreamer->EmitZeros(1);
      }
    }
  } else {
    for (unsigned I = 0, E = CDS->getNumElements(); I != E; ++I)
...
static void emitGlobalConstantArray(const DataLayout &DL,
  ...
  if (Value != -1) {
    ...
    for (unsigned i = 0, e = CA->getNumOperands(); i != e; ++i) {
      ...
      // SICREAL - HACK - SICHACK - 24 bit ints in structs / arrays
      int ElementByteSize = DL.getTypeAllocSize(CA->getOperand(i)->getType());
      if (ElementByteSize == 4 && strcmp(AP.TM.getTarget().getName(), "sic") == 0) {
        AP.OutStreamer->EmitZeros(1);
      }
...
static void emitGlobalConstantStruct(const DataLayout &DL,
  ...
  for (unsigned i = 0, e = CS->getNumOperands(); i != e; ++i) {
    ...
    // SICREAL - HACK - SICHACK - 24 bit ints in structs / arrays
    if (FieldSize == 4 && strcmp(AP.TM.getTarget().getName(), "sic") == 0) {
      FieldSize = 3;
    }
